...
def process_directory(directory):
        ...
        for file in files:
            ...
            if file_extension in audio_formats:
                ...
                if file_extension == ".flac":
                    alac_path = os.path.join(
                        ALAC_DIR, os.path.relpath(file_path, directory)
                    )
    ...
    for fmtx, files in format_files.items():
        with open(f"{OUTPUT_DIR}/{fmtx[1:]}_files.txt", "w") as f:
            f.write("\n".join(files))

    top_directories = nlargest(10, directory_sizes.items(), key=lambda x: x[1])

    print("Audio file counts:")
    for fmtx, count in audio_counts.items():
        print(f"{fmtx}: {count}")

    print("\nOther file counts:")
    for fmtx, count in other_counts.items():
        print(f"{fmtx}: {count}")
...
parser = argparse.ArgumentParser(
    description="Process directory and generate file statistics."
)
