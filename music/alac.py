...
def convert_file(file, input_dir, output_dir, error_log_path):
    ...
    if not os.path.exists(output_file):
        try:
            ...
            subprocess.run(
                ["ffmpeg", "-i", file, "-acodec", "alac", output_file], check=True
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    pool.starmap(
                        ...
                        [
                            (
                                ...
                                input_dir,
                                output_dir,
                                os.path.join(temp_dir, f"error_log_{i}.log"),
                            )
            ...
            with open(error_log_path, "w") as error_log:
                for i in range(len(flac_files)):
                    process_error_log_path = os.path.join(
                        temp_dir, f"error_log_{i}.log"
                    )
